...
//****SLURPS*****//
  ...
  // Create a new slurp in an id-specified restaurant
  app.post('/api/neighborhood/:neighborhood_id/restaurants/:restaurant_id/slurps', function(req, res) {
    var restaurantId = req.params.restaurant_id;
      console.log(restaurantId);

    var newSlurp = req.body.slurps;
      console.log(newSlurp);
    ...
    db.Neighborhood.findById(req.params.neighborhood_id, function(err, foundNeighborhood){
      for (var i = 0; i < foundNeighborhood.restaurants.length; i++){
        ...
        if (restaurantId === idString) {
          foundNeighborhood.restaurants[i].slurps.push(newSlurp);
          console.log('success!');
        }
      }
      foundNeighborhood.save(function(err){
        ...
        res.json({ message: "new slurp for Restaurant created!!" });
      })
    });
  });
